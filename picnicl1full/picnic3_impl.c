...
static void aux_mpc_sbox(const uint32_t* in, const uint32_t* out, randomTape_t* tapes, paramset_t* params)
{
    ...
    for (size_t i = 0; i < params->numSboxes * 3; i += 3) {
        ...
        bitNumber += 2;
        uint8_t a;
        getBitFromWordArray((uint64_t*) in, &bitNumber, (uint64_t*) &a);
        bitNumber -= 1;
        uint8_t b;
        getBitFromWordArray((uint64_t*) in, &bitNumber, (uint64_t*) &b);
        bitNumber -= 1;
        uint8_t c;
        getBitFromWordArray((uint64_t*) in, &bitNumber, (uint64_t*) &c);

        bitNumber += 2;
        uint8_t d;
        getBitFromWordArray((uint64_t*) out, &bitNumber, (uint64_t*) &d);
        bitNumber -= 1;
        uint8_t e;
        getBitFromWordArray((uint64_t*) out, &bitNumber, (uint64_t*) &e);
        bitNumber -= 1;
        uint8_t f;
        getBitFromWordArray((uint64_t*)out, &bitNumber, (uint64_t*) &f);
        bitNumber += 3;
...
static void computeAuxTape(randomTape_t* tapes, uint8_t* inputs, paramset_t* params)
{
    ...
    for (uint32_t r = params->numRounds; r > 0; r--) {
        ...
        xor_array((uint64_t*)x, (uint64_t*)x, (uint64_t*)roundKey, (uint64_t*)&params->stateSizeWords);
                           ...
                           msgs_t* msgs, const uint32_t* plaintext, const uint32_t* pubKey, paramset_t* params)
{
    ...
    xor_array((uint64_t*)state, (uint64_t*)roundKey, (uint64_t*)plaintext, (uint64_t*)&params->stateSizeWords);      // state = plaintext + roundKey
    ...
    for (uint32_t r = 1; r <= params->numRounds; r++) {
        ...
        xor_array((uint64_t*)state, (uint64_t*)state, (uint64_t*)RConstant(r - 1, params), (uint64_t*)&params->stateSizeWords);  // state += RConstant
        matrix_mul(roundKey, maskedKey, KMatrix(r, params), params);
        xor_array((uint64_t*)state, (uint64_t*)roundKey, (uint64_t*)state, (uint64_t*)&params->stateSizeWords);      // state += roundKey
    }
                 ...
                 size_t messageByteLength, signature2_t* sig, paramset_t* params)
{
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        ...
        xor_array((uint64_t*)maskedKey, (uint64_t*)maskedKey, (uint64_t*)privateKey, (uint64_t*)&params->stateSizeWords);
