...
int main(void) {
	...
	for(int k=0; k<NUM_ROUNDS; k++) {
		...
		for(int j = 0; j< 64; j++){
			localViews[k][0].x[j] = 0;
			localViews[k][1].x[j] = 0;
			localViews[k][2].x[j] = 0;
		}
		for(int j = 0; j< 370; j++){
			localViews[k][0].y[j] = 0;
			localViews[k][1].y[j] = 0;
			localViews[k][2].y[j] = 0;
		}
	...
	*/
	...
	for (int j = 0; j < 8; j++) {
		...
		aux = &finalHash;
		printf("%u; %u; %u; %u;\n", *(aux + j*4), *(aux + j*4 + 1), *(aux + j*4 + 2), *(aux + j*4 + 3));
	}
	printf("\n\n\n");
